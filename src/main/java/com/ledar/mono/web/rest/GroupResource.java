...
/**
 * REST controller for managing {@link com.ledar.mono.domain.Group}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class GroupResource {
    ...
    public ResponseEntity<Group> partialUpdateGroup(
        ...
        Optional<Group> result = groupRepository
            ...
            .map(existingGroup -> {
                if (group.getGroupLeaderStaffId() != null) {
                    existingGroup.setGroupLeaderStaffId(group.getGroupLeaderStaffId());
                }
                if (group.getDepartmentId() != null) {
                    existingGroup.setDepartmentId(group.getDepartmentId());
                }
